...
import { PaymentCard } from '../../schema/paymentCard.schema';

@Injectable({})
export class PaymentCardLogService extends LogDBService {
  constructor(
    @InjectModel(EventLog.name)
    protected readonly EventLogModel: Model<EventLog>,
  ) {
    super(EventLogModel);
    this.modelName = PaymentCard.name;
  }
  ...
  async doUpdate(params) {
    ...
    try {
      ...
      //compare name
      result = this.compareChangedData(self, docToUpdate, 'isPrimary', result);
